# Logistic model for team project - cross validation + oversampling
...
for (test in cv) {
  ...
  data_train_balanced <- ovun.sample(adopter ~ . , data = data_train, 
                                   method = "over", 
                                   N = 2*table(data_train$adopter)[1])$data
  
  logit_model <- glm(adopter ~ . , data = data_train_balanced, family = "binomial")
